...
test('agent and sensor registrations are explicit, disabled, and non-mutating by default', () => {
  ...
  assert.equal(WORKFLOW_REGISTRATIONS.length, 5)
  assert.ok(WORKFLOW_REGISTRATIONS.filter((registration) => registration.name !== 'opportunity_review')
    .every((registration) => registration.status === 'implemented'))
  assert.equal(WORKFLOW_REGISTRATIONS.find((registration) => registration.name === 'opportunity_review')?.status, 'foundation')
