...
export function SiteFooter({ containerClassName }: SiteFooterProps) {
  ...
  return (
    <footer className="border-t border-border-subtle bg-white/95 text-text-primary">
      <div className={`${containerClass} py-12 sm:py-14`}>
        <div className="grid gap-10 md:grid-cols-[1.1fr_repeat(3,minmax(0,1fr))]">
          <div className="space-y-3 text-sm text-text-secondary">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-brand-copper">Vendor hub</p>
            <p className="text-lg font-semibold text-text-primary">Nested Objects</p>
        ...
        <div className="mt-12 flex flex-col items-center gap-3 border-t border-border-subtle pt-6 text-sm text-text-secondary">
          <div className="flex items-center gap-3 rounded-full border border-border-strong bg-surface-subtle px-4 py-2 shadow-brand-soft">
            <Image src="/logo-light.svg" alt="Nested Objects logo" width={28} height={28} />
            <div className="flex flex-col leading-tight">
              <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-copper">Nested Objects</span>
              <span className="text-sm font-semibold text-text-primary">Vendor Hub</span>
            </div>
          </div>
          <p className="text-[11px] text-text-muted">© {new Date().getFullYear()} Nested Objects LLC. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
