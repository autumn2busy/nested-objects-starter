...
import { ArrowLeft, ArrowRight, CheckCircle2, Clock, Eye } from 'lucide-react'
...
export function BlogPostArticle({ post, mode }: BlogPostArticleProps) {
        ...
        <main className="min-h-screen bg-brand-sand text-slate-950">
            {isPreview && (
                <section className="border-b border-amber-200 bg-amber-50">
                    <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-4 text-sm text-amber-900 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
                        <div className="flex items-start gap-3">
                            <Eye className="mt-0.5 h-5 w-5 shrink-0" />
                            <div>
                                <p className="font-bold">Preview mode: {statusLabel(post.status)}</p>
                                <p className="mt-1 leading-6">
                                    This route is noindex and excluded from the sitemap. Edit the post in
                                    {' '}
                                    <span className="font-mono text-xs">apps/web-members/lib/blog.ts</span>
                                    {' '}
                                    before approving it.
                                </p>
                            </div>
                        ...
                        {post.status === 'approved' && (
                            <Link
                                href={`/blog/${post.slug}`}
                                className="inline-flex items-center justify-center rounded-full bg-amber-900 px-4 py-2 text-xs font-bold text-white transition hover:bg-amber-800"
                            >
                                Open live article
                            </Link>
                        )}
                    ...
                    <aside className="space-y-5 lg:sticky lg:top-8 lg:self-start">
                        <div className="rounded-2xl border border-brand/20 bg-white p-5 shadow-sm">
                            <p className="text-xs font-bold uppercase tracking-widest text-brand">
                                {isPreview ? 'Review Status' : 'Human Review'}
                            </p>
                            <p className="mt-2 text-sm font-semibold text-slate-950">
                                {post.review.approvedAt
                                    ? `Approved ${post.review.approvedAt}`
                                    : `Current status: ${statusLabel(post.status)}`}
                            </p>
                            <p className="mt-1 text-xs leading-5 text-slate-500">
                                {isPreview
                                    ? 'Use this preview to check the article before changing it to approved.'
