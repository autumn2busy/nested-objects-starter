        ...
        return (
            <div className="flex h-screen items-center justify-center bg-brand-micra">
                <div className="text-center">
                    ...
                    <Link href="/challenges" className="text-brand-copper hover:underline mt-2 inline-block">Return to Track</Link>
                </div>
            </div>
        )
    }
        ...
        <div className="flex h-screen flex-col lg:flex-row bg-slate-50 overflow-hidden">
            ...
            <aside className="w-full lg:w-80 border-r border-slate-200 bg-white flex flex-col shrink-0">
                <div className="p-4 border-b border-slate-100 flex items-center gap-2">
                    <Link href="/challenges" className="text-slate-400 hover:text-slate-800 transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {advancedFieldInspectionModules.map((module, index) => {
                        const isActive = module.id === moduleId
                        const isCompleted = completedModules.includes(module.id)

                        // Strict Locking Logic for Advanced:
                        // Index 0 (Mod 5) requires Basic Mod 4 (last basic) to be done.
                        // Other indices require previous Advanced mod to be done.
                        let isUnlocked = false
                        if (index === 0) {
                            const lastBasic = basicFieldInspectionModules[basicFieldInspectionModules.length - 1]
                            isUnlocked = completedModules.includes(lastBasic.id)
                        } else {
                            isUnlocked = completedModules.includes(advancedFieldInspectionModules[index - 1].id)
                        }
                        ...
                        if (isUnlocked) {
                            return (
                                <Link key={module.id} href={`/challenges/advanced/${module.id}`}>
                                    {content}
                                </Link>
                            )
                        }
                            ...
                            <div className="bg-emerald-900 text-white rounded-2xl p-6 shadow-xl sticky top-6">
                                ...
                                {requiresQuiz ? (
                                    ...
                                    <>
                                        ...
                                        {nextModule ? (
                                            <Link
                                                href={`/challenges/advanced/${nextModule.id}`}
