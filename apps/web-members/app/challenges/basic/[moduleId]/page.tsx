...
export default function ModulePlayerPage() {
    ...
    // We'll rely on sidebar/inspector-dashboard to prevent navigation, but handle valid render here.
            ...
            <aside className="w-full lg:w-80 border-r border-slate-200 bg-white flex flex-col shrink-0">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {basicFieldInspectionModules.map((module, index) => {
                        ...
                        if (isUnlocked) {
                            return (
                                <Link key={module.id} href={`/challenges/basic/${module.id}`}>
                                    {content}
                                </Link>
                            )
                        }
                            ...
                            <div className="bg-emerald-900 text-white rounded-2xl p-6 shadow-xl sticky top-6">
                                ...
                                {isModuleCompleted ? (
                                    <div className="space-y-4">
                                        ...
                                        {nextModule ? (
                                            <Link
                                                href={`/challenges/basic/${nextModule.id}`}
                                    ...
                                    <>
                                        {currentModule.quiz && currentModule.quiz.length > 0 ? (
                                            <>
                                                ...
                                                <Link
                                                    href={`/challenges/basic/${moduleId}/quiz`}
                                                    className={cn(buttonVariants({ className: "w-full bg-emerald-400 hover:bg-emerald-300 text-emerald-950 font-bold" }))}
                                                >
                                                    Take Quiz
                                                </Link>
