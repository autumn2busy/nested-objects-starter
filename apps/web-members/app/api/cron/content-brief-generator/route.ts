...
export async function GET(request: Request) {
  ...
  const report = runContentBriefGenerator()
  ...
  if (dryRun) {
    return NextResponse.json({
      ...
      commit: {
        committed: false,
        reason:
          'Manual content brief generator runs do not commit unless commit=1 is provided. Vercel cron runs still commit automatically.',
      },
    })
  }
