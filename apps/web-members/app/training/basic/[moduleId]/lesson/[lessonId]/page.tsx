...
export default function LessonPage() {
    ...
    const handleComplete = async () => {
        ...
        if (nextLesson) {
            router.push(`/training/basic/${moduleId}/lesson/${nextLesson.id}`)
        } else {
            router.push(`/training/basic/${moduleId}`)
        }
    }
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {moduleData.lessons?.map((lesson, idx) => {
                        ...
                        return (
                            <Link
                                key={lesson.id}
                                href={`/training/basic/${moduleId}/lesson/${lesson.id}`}
                                className={cn(
            ...
            <main className="flex-1 max-w-4xl mx-auto w-full p-4 lg:p-12 pb-32">
                ...
                <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 lg:pl-80 z-30">
                    <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
                        {prevLesson ? (
                            <Link
                                href={`/training/basic/${moduleId}/lesson/${prevLesson.id}`}
